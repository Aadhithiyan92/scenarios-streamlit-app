...
# Set page config
st.set_page_config(layout="wide", page_title="BEV Adoption Scenario Analysis with Historical RMSE")
...
try:
    ...
    def calculate_historical_rmse(data, params):
        """Calculate RMSE from historical data"""
        historical_bev = V_BEV_data[-5:]
        ...
        rmse = np.sqrt(np.mean((historical_bev - predicted_bev)**2)) / np.mean(historical_bev)
        
        return rmse

    # Calculate historical RMSE
    historical_rmse = calculate_historical_rmse(data, params)
    
    def run_scenario_with_error(base_params, growth_ratio, incentive_ratio, rmse=historical_rmse):
        """Run scenario with Monte Carlo error simulation using historical RMSE"""
    ...
    # Create main title
    st.title("BEV Adoption Scenario Analysis with Historical Error")
    st.write(f"Historical Model RMSE: {historical_rmse:.1%}")
    ...
    with tab1:
        ...
        fig.update_layout(
            title=f'BEV Adoption Scenarios (Historical RMSE: {historical_rmse:.1%})',
            xaxis_title='Year',
    ...
    with st.sidebar:
        ...
        st.write(f"Historical RMSE: {historical_rmse:.1%}")
        ...
        if st.button("Download Results"):
            ...
            results_df.to_excel('bev_scenarios_historical_rmse.xlsx', index=False)
